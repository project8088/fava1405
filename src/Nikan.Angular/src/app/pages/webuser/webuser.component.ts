...
export class WebUserComponent implements OnInit, OnDestroy{
  ...
  getLastApiHelp() {
    ...
    this.dataService.get(ServerApis.getLastNews, {}).subscribe(response => {
      ...
      for (var i in arr) {
        this.menuItems.push({ name: arr[i].title, icon: 'fa fa-barcode', url: '/webuser/help-details/' + arr[i].id });
      }

      this.menuItems.push({ name: 'تغییر کلمه عبور', icon: 'fa fa-key', url: '/webuser/change-password' });
