...
export class CompanyBaseInfoComponent implements OnInit {
  ...
  constructor(
    ...
    this.form = this.fb.group({ 
      ...
      englishName: [null, [Validators.required, Validators.maxLength(40), this.customValidators.checkEnglishCharacters]],
      establishedYear: [null, [Validators.required, Validators.maxLength(4), Validators.minLength(4)]],
  ...
  getAddressInfo() {
    this.loading = true;
    this.dataService.get(ServerApis.getCompanyBaseInfo, {
  ...
  save() {
    ...
    let dataToPost = {
      ...
      activityLicenseDate: form.activityLicenseDate ? this.dataService.formatDate(form.activityLicenseDate):null,
    ...
    this.dataService.post(ServerApis.updateCompnayBaseInfo, dataToPost).subscribe(response => {
      this.isSaving = false;
