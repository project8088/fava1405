...
export class CompanyMainInfoComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.mainForm = this.fb.group({
      slagUrl: [null, [Validators.required, Validators.maxLength(40), this.customValidators.checkEnglishWithoutSpace]],
      content: [null, []],
  ...
  getAddressInfo() {
    this.loading = true;
    this.dataService.get(ServerApis.getCompanyMainInfo, {
      ...
      if (response.isSuccess && response.data) {
        ...
        this.mainForm.setValue({
          ...
          numberOfEmployees: response.data.numberOfEmployees ? response.data.numberOfEmployees:0
        });
        setTimeout(() => {
          this.loadCkEditor(this.mainForm.get('content').value);
        }, 1000);
  ...
  save() {
    ...
    this.dataService.post(ServerApis.updateCompanyMainInfo, dataToPost).subscribe(response => {
      this.isSaving = false;
