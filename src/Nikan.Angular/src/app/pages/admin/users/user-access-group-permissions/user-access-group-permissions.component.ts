...
export class AdminUserAccessGroupPermissionsComponent implements OnInit {
  ...
  getuserRoleList() {
    this.loading = true;
    this.dataService.get(ServerApis.getAllUserPermissionGroup, {
      id: this.userId
    }).subscribe(response => {
      this.loading = false;
  ...
  save() {
    ...
    this.dataService.post(ServerApis.addUserToPermissionGroup, dataToPost).subscribe(response => {
      this.isSaving = false;
  ...
  delete(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.dataService.get(ServerApis.removeUserPermissionGroup, { userId: row.userId, permissionGroupId: row.permissionGroupId }).subscribe(response => {
          row.loading = false;
          if (response.isSuccess) {
