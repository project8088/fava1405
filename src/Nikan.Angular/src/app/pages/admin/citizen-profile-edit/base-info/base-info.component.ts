...
export class AdminCitizenBaseInfoComponent implements OnInit {
  ...
  getPersonalInfo() {
    this.loading = true;
    this.dataService.get(ServerApis.getCitizenBaseInfoByAdmin, { userCode: this.userCode }).subscribe(
      (response) => {
        this.loading = false;
  ...
  save() {
    ...
    this.dataService.post(ServerApis.updteCitizenByAdmin, dataToPost).subscribe(response => {
      this.isSaving = false;
