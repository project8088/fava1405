...
export class AdminPaySettingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loading = true;
    this.dataService.get(ServerApis.getFinancialSettings).subscribe(response => {
      this.loading = false;
  ...
  saveSetting() {
    ...
    this.dataService.post(ServerApis.updateFinancialSettings, params).subscribe(response => {
      this.isSaving = false;
