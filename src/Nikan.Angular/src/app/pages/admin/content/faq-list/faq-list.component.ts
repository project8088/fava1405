...
export class AdminFaqListComponent implements OnInit, AfterViewInit {
  loading: boolean;
  displayedColumns: string[] = ['row', 'title', 'questionGroup','viewCount','isActive', 'operation'];
  ...
  getList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.getFaqList, {}).subscribe(response => {
  ...
  delete(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeFaq,{ id: row.id }).subscribe(response => {
          if (response.isSuccess) {
