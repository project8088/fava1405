...
export class ManageAttachmentDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataService.get(ServerApis.getAttachmentsForAdmin, { guid: this.data.guid }).subscribe(response => {
      ...
      if (response.isSuccess) {
        ...
        this.attachments.forEach(item => {
          ...
          item.isImage = (["jpg", "jpeg", "png", "gif", "tif", "bmp"].indexOf(item.fileExtension.toLowerCase()) > -1) ? true : false;
  ...
  deleteAttachment(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        row.loading = true;
        this.dataService.get(ServerApis.removeAttachment, { id: row.id }).subscribe(response => {
          row.loading = false;
          if (response.isSuccess) {
