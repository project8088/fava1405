...
export class CardFreeRequestCardListComponent implements OnInit, AfterViewInit {
  loading: boolean;
  displayedColumns: string[] = ['row', 'discountTitle', 'cardType','group', 'creationDate', 'creationBy',
    'letterNumber', 'freeCardApplicantOrganization', 'accepted',   'operation'];
  ...
  delete(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeRequestFreeCard, { id: row.id }).subscribe(response => {
  ...
  acceptedRequest(item) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        item.loading = true;
        this.dataService.get(ServerApis.acceptedRequestFreeCard, {
          id: item.id,
        }).subscribe(response => {
          item.loading = false;
          if (response.isSuccess) {
