...
export class CardCitizenCardExportSearchComponent implements AfterViewInit {
  ...
  sendToPrint(item) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        item.loading = true;
        this.dataService.get(ServerApis.sendToPrint, {
          id: item.id,
        }).subscribe(response => {
          item.loading = false;
  ...
  delete(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        row.loading = true;
        this.dataService.get(ServerApis.removeExportCard, {
          id: row.id,
        }).subscribe(response => {
          row.loading = false;
          if (response.isSuccess) {
  ...
  opennewExportDialog() {
    this.matDialog.open(CardNewExportCardDialogComponent, {
      panelClass: 'custom-dialog',
      minWidth: '600px',
      data: { }
