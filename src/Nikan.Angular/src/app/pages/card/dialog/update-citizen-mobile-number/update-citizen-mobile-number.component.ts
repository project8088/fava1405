...
export class CardUpdateCitizenMobileNumberDialogComponent implements OnInit {
  ...
  getUserInfo() {
    this.loading = true
    //todo
    this.dataService.get(ServerApis.getShortCitizenInfoByCard, { userCode: this.userCode }).subscribe(response => {
      this.loading = false;
  ...
  saveInfo() {
    ...
    this.isSaving = true;
    this.dataService.post(ServerApis.updateCitizenMobileNumberByCard,
      {
