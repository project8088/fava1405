...
export class DataService {
  ...
  downloadFile(url: string, params: any, type = 'application/vnd.ms-excel', fileName = 'export.xls') {
  
     
    this.http.get<ApiResult<any>>(url, {
      params,
  ...
  protected  getBaseEnums(): Observable<any> {
    return this.http.get(ServerApis.getlistBase)
      .pipe(
        map((response: any) => {
  ...
  getEnums(): Observable<any> {
    ...
    if (enums)
      ...
      return this.getBaseEnums();
  }


  getTextOfEnum(objectName, key): Observable<string> {
    return this.getEnums().pipe(map( response => {
      let enums = response;
  ...
  protected getSiteSettingFromServer(): Observable<SiteSettingViewModel> {
    return this.http.get(ServerApis.getSiteInfo)
      .pipe(
  ...
  getSetting(): Observable<SiteSettingViewModel> {
    ...
    if (setting)
      ...
      return this.getSiteSettingFromServer();
  }
