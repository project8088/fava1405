...
export class UploaderComponent implements OnInit {
  ...
  //['application/pdf']
  @Input('accept') accept: string[] = ['image/png', 'image/jpg', 'image/gif', 'image/jpeg', 'image/bmp'];
    ...
    switch (output.type) {
      ...
      case 'done':
        if (output.file.responseStatus == 200 && output.file.response.isSuccess) {
          ...
          let msg = output.file.response.messages ? output.file.response.messages : 'متاسفانه سرور با خطا مواجه شده است.';
