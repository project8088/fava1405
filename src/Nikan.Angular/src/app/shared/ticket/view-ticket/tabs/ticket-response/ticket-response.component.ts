...
export class TicketResponseComponent implements OnInit, AfterViewInit {
  ...
  getTicketInfo() {
    this.loading = true;
    this.dataService.get(ServerApis.getTicketById, { id: this.id }).subscribe(response => {
      this.loading = false;
  ...
  closeTicket(isClosed) {
    Swal.fire({
      text: isClosed ? 'آیا برای بستن تیکت اطمینان دارید؟' : 'آیا برای باز کردن تیکت اطمینان دارید؟',
      ...
      if (result.value) {
        this.loading = true;
        this.dataService.post(ServerApis.updateTicketsStatues, {
          TicketId: this.id,
          isClosed: isClosed
        }).subscribe(response => {
          this.loading = false;
  ...
  sendAnswer() {
    ...
    this.isSending = true;
    this.dataService.post(ServerApis.sendAnswerTicket, {
      TicketId: this.id,
