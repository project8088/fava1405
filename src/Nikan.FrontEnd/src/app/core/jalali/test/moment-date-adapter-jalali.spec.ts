...
describe('Jalali MomentDateAdapter', () => {
  ...
  it('should parse number', () => {
    ...
    // tslint:disable-next-line:no-non-null-assertion
    expect(adapter.parse(timestamp, 'jMM/jDD/jYYYY')!.format()).toEqual(moment(timestamp).format());
  });

  it('should parse Date', () => {
    const date = new Date(2017, JAN, 1);
    // tslint:disable-next-line:no-non-null-assertion
    expect(adapter.parse(date, 'jMM/jDD/jYYYY')!.format()).toEqual(moment(date).format());
  });
  ...
  it('should parse invalid value as invalid', () => {
    ...
    expect(adapter.isValid(d)).toBe(false, 'Expected to parse as "invalid date" object');
  });

  it('should format date according to given format', () => {
    expect(adapter.format(moment([2017, JAN, 2]), 'jMM/jDD/jYYYY')).toEqual('10/13/1395');
    expect(adapter.format(moment([2017, JAN, 2]), 'jDD/jMM/jYYYY')).toEqual('13/10/1395');
  });
  ...
  it('should add years - second', () => {
    expect(adapter.addCalendarYears(moment('1396/01/01', 'jYYYY/jMM/jDD'), 1).format()).toEqual(
      moment('1397/01/01', 'jYYYY/jMM/jDD').format(),
    );
    expect(adapter.addCalendarYears(moment('1396/01/01', 'jYYYY/jMM/jDD'), -1).format()).toEqual(
      moment('1395/01/01', 'jYYYY/jMM/jDD').format(),
    );
  });

  it('should add months - second', () => {
    expect(adapter.addCalendarMonths(moment('1396/01/01', 'jYYYY/jMM/jDD'), 1).format()).toEqual(
      moment('1396/02/01', 'jYYYY/jMM/jDD').format(),
    );
    expect(adapter.addCalendarMonths(moment('1396/01/01', 'jYYYY/jMM/jDD'), -1).format()).toEqual(
      moment('1395/12/01', 'jYYYY/jMM/jDD').format(),
    );
  });

  it('should respect month length differences when adding months', () => {
    expect(adapter.addCalendarMonths(moment([2017, SEP, 22]), 1).format()).toEqual(
      moment([2017, OCT, 22]).format(),
    );
    expect(adapter.addCalendarMonths(moment([2017, SEP, 22]), -1).format()).toEqual(
      moment([2017, AUG, 22]).format(),
    );
  });

  it('should add days - second', () => {
    expect(adapter.addCalendarDays(moment('1396/01/01', 'jYYYY/jMM/jDD'), 1).format()).toEqual(
      moment('1396/01/02', 'jYYYY/jMM/jDD').format(),
    );
    expect(adapter.addCalendarDays(moment('1396/01/01', 'jYYYY/jMM/jDD'), -1).format()).toEqual(
      moment('1395/12/30', 'jYYYY/jMM/jDD').format(),
    );
  });
