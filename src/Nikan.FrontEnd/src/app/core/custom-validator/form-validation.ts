...
export class CustomFormValidators {
  ...
  checkMobileNumber(control: AbstractControl) {
    ...
    var validNumber = ['901', '902', '903', '904', '905', '930', '933', '935', '936', '937', '938', '939',  //irancel
      '910', '911', '912', '913', '914', '915', '916', '917', '918', '919', '990', '991', '992', '993', '994', '995', '996', '997', '998', '999',//MCI
 ...
 */
  ...
  strongPassword(control: AbstractControl) {
    if (!control.value)
      return null;
    var word = control.value;
    var isValid = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%?&^#*()_\-=+])[A-Za-z\d$@$!%?&^#*()_\-=+]{6,}$/.test(word);
    if (isValid)
      return null
    else
  ...
  website(control: AbstractControl) {
    if (!control.value)
      return null;
    var word = control.value;
    var Valid = /(https?:\/\/)?(www\.)[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,4}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)|(https?:\/\/)?(www\.)?(?!ww)[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,4}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g.test(word);
    if (Valid)
      return null
    else
      return { invalid: true };
