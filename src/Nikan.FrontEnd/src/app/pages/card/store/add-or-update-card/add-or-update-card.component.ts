...
export class OrderAddOrUpdateCardComponent implements OnInit, AfterViewInit {
  isUpdate: boolean;
  id: string;
  storeForm: FormGroup;
  ...
  ngOnInit(): void {
    this.dataService.get(ServerApis.getCardTypeBaseList, {}).subscribe(response => {
      if (response.isSuccess)
        this.cardTypeList = response.data ? response.data : [];
    });
  }
  ...
  save() {
    ...
    if (this.htmlEditor.getData())
      params.buyCardDescription = this.htmlEditor.getData();
    this.isSaving = true;
    this.dataService.post(ServerApis.addOrUpdateCard, params).subscribe(response => {
      this.isSaving = false;
      if (response.isSuccess) {
