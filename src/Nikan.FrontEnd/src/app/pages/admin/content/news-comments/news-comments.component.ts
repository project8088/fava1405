...
export class AdminNewsCommentsComponent implements  OnInit {
  ...
  displayedColumns: string[] = ['row', 'userIP', 'emailAddress', 'fullName', 'commentMessage', 'isPublish', 'operation'];
  ...
  getCommentList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.getNewsComments, { id: this.newsId }).subscribe(response => {
  ...
  publishComment(row) {
    ...
    Swal.fire({
      title: 'تغییر وضعیت',
      html: '<p>آیا برای انتشار خبر اطمینان دارید؟</p> <p><small class="text-info">' + row.commentMessage+'</small></p>',
  ...
  publish(row, IsPublish) {
    this.dataService.post(ServerApis.publishComment, {
      commentId: row.id,
      IsPublish: IsPublish
    }).subscribe(response => {
      if (response.isSuccess) {
