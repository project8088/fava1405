...
export class AdminSlideShowListComponent implements OnInit, AfterViewInit {
  loading: boolean;
  displayedColumns: string[] = ['row','imageUrl', 'caption', 'description','isActive', 'operation'];
  ...
  getList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.getAllSlideShow, {}).subscribe(response => {
  ...
  delete(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeSlideShow,{ id: row.id }).subscribe(response => {
          if (response.isSuccess) {
  ...
  openDialog(row) {
    this.matDialog.open(AdminAddOrUpdateSlideShowDialogComponent, {
