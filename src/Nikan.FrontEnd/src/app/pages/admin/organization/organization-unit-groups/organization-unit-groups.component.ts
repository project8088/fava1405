...
export class AdminOrganizationUnitGroupsComponent implements OnInit {
  ...
  getuserRoleList() {
    this.loading = true;
    this.dataService.get(ServerApis.getAllUnitGroups, {
      id: this.unitId
    }).subscribe(response => {
      this.loading = false;
  ...
  save() {
    ...
    this.dataService.post(ServerApis.addGroupToUnitGroup, dataToPost).subscribe(response => {
      this.isSaving = false;
  ...
  delete(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.dataService.get(ServerApis.removeUnitGroup, { unitId: row.unitId, groupId: row.groupId }).subscribe(response => {
          row.loading = false;
          if (response.isSuccess) {
