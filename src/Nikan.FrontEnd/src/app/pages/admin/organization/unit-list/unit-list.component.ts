...
export class AdminUnitListComponent implements AfterViewInit {
  ...
  getList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.getAllUnit, { orgId: this.organizationId }).subscribe(response => {
  ...
  save() {
    ...
    this.dataService.post(ServerApis.addOrUpdateUnit, this.frm.value).subscribe(response => {
      this.isSaving = false;
      if (response.isSuccess) {
  ...
  delete(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeUnit, {
          id: row.id,
        }).subscribe(response => {
          if (response.isSuccess) {
