...
export class AdminRefundExcelBatchFileDetailsComponent implements AfterViewInit {
   loading: boolean;
   isSaving: boolean;

   displayedColumns: string[] = ['row', 'citizen', 'nationalCode', 'orderId', 'saleReferenceId', 'refundAmount', 'totalRefundAmount','refundCardNumber' ];
  ...
  getList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.refundImportFileDetails, { importId: this.importId}).subscribe(response => {
  ...
  getCardNumber() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.getCardsNumber, {
   ...
   save() {
     ...
     this.isSaving = true;
     this.dataService.post(ServerApis.addRefundAccess, form).subscribe(response => {
       this.isSaving = false;
       if (response.isSuccess) {
   ...
   delete() {
     Swal.fire({
       ...
       if (result.value) {
         this.dataService.get(ServerApis.removeRefundImportFile, { importId: this.importId }).subscribe(response => {
           if (response.isSuccess) {
