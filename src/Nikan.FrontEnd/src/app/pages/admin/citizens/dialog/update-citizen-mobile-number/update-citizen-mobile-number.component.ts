...
export class AdminUpdateCitizenMobileNumberDialogComponent implements OnInit {
  isSaving: boolean;
  userForm: FormGroup;
  userCode: string;
  ...
  getUserInfo() {
    this.loading = true
    //todo
    this.dataService.get(ServerApis.getShortCitizenInfoByAdmin, { userCode: this.userCode }).subscribe(response => {
      this.loading = false;
  ...
  saveInfo() {
    ...
    this.isSaving = true;
    this.dataService.post(ServerApis.updteCitizenMobileNumberByAdmin,
      {
