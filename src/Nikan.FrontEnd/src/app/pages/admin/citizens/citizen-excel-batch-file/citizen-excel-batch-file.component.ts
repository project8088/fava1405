...
export class AdminCitizenExcelBatchFileListComponent implements AfterViewInit {
  loading: boolean;
   displayedColumns: string[] = ['row', 'importId', 'fileName', 'importBy', 'onDate','count', 'isConfirm','operation'];
  ...
  getList() {
    ...
    this.data = [];
    this.dataService.get(ServerApis.citizenImportFileList, {}).subscribe(response => {
  ...
  delete(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeCitizenImportFile, { importId: row.importId }).subscribe(response => {
          if (response.isSuccess) {
   ...
   confirm(item) {
     ...
     Swal.fire({
       ...
       if (result.value) {
         item.loading = true;
         this.dataService.get(ServerApis.confirmCitizenfileexcel, {
