...
export class AdminUserAccessIpComponent implements OnInit {
  ...
  getWebUserAccessRangeIp() {
    this.loading = true;
    this.dataService.get(ServerApis.getWebUserAccessRangeIp, {
      userId: this.userId
    }).subscribe(response => {
      this.loading = false;
  ...
  save() {
    ...
    this.dataService.post(ServerApis.addWebUserAccessRangeIp, dataToPost).subscribe(response => {
      this.isSaving = false;
  ...
  delete(row) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.dataService.get(ServerApis.deleteUserAccessRangeIp, { userId: row.userId, ipId: row.id }).subscribe(response => {
          row.loading = false;
          if (response.isSuccess) {
