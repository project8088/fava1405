...
export class AdminUserGroupsComponent implements OnInit {
  ...
  getList() {
    this.loading = true;
    this.dataService.get(ServerApis.getAllUserGroups, { }).subscribe(response => {
      this.loading = false;
  ...
  openAddUserDialog() {
    this.matDialog.open(AdminAddUserGrousDialogComponent, {
  ...
  deleteUserGroup(row) { 
    Swal.fire({
      ...
      if (result.value) {
        this.dataService.get(ServerApis.removeUserGroups, {
          id: row.id,
        }).subscribe(response => {
