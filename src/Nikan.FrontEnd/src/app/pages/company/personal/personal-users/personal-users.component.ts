...
export class CompanyPersonalUsersComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.user = this.authService.currentUserValue;
       this.displayedColumns = ['row', 'imageUrl', 'personelCode', 'displayName', 'fatherName', 'nationCode', 'userCompany', 'organizationalPosition', 'mobileNumber', 'cellNumber', 'email', 'city', 'isManagementMembers', 'operation'];
  ...
  getList() {
    ...
    if (this.user.isAdmin)
      url = ServerApis.allCompanyPersonel
  ...
  removePersonal(row) {
    Swal.fire({
      ...
      if (result.value) {
        row.loading = true;
        this.dataService.get(ServerApis.removePersonel, { id: row.id }).subscribe(response => {
          row.loading = false;
