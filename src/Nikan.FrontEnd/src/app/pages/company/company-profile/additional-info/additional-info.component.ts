...
export class CompanyAdditionalInfoComponent implements OnInit, AfterViewInit {
  ...
  getAdditionalInfo() {
    this.loading = true;
    this.dataService.get(ServerApis.getAdditionalInfo, {
  ...
  save() {
    ...
    if (form.contractOnDate)
      form.contractOnDate = this.dataService.formatDate(form.contractOnDate);
    ...
    let dataToPost = form;
    this.dataService.post(ServerApis.updateAdditionalInfo, dataToPost).subscribe(response => {
      this.isSaving = false;
