...
export class FaqListComponent implements OnInit {
    ...
    this.dataService.get(ServerApis.getFaqGroups, {}).subscribe(response => {
      ...
      if (response && response.isSuccess) {
        ...
        if (this.faqGroups.length > 0)
         this.getFaqList(this.faqGroups[0]);
      } else {
  ...
  getFaqList(item) {
    ...
    this.dataService.get(ServerApis.getFaqList,{ groupId: item.key }).subscribe(response => {
  ...
  getDetails(item: any) {
    if (item.showDetails === true) {
      ...
      item.showDetails = true;
      item.loading = true;
      this.dataService.get(ServerApis.getFaq,{ id: item.id }).subscribe(response => {
        item.loading = false;
        if (response && response.isSuccess) {
