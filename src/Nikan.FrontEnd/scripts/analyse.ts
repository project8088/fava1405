...
const modules: { name: string; path: string; ngModule?: any }[] = [];
const components: { name: string; path: string; ngComponent?: any }[] = [];
...
modules.forEach((moduleInfo) => {
  ...
  const importedModulesNames =
    moduleDecoratorArgs?.imports?.map((imp: any) => imp.getText().replace('Module', '')) || [];
  const declaredComponentsNames =
    moduleDecoratorArgs?.declarations?.map((dec: any) => dec.getText().replace('Component', '')) ||
    [];

  // بررسی کامپوننت‌های تعریف شده در این ماژول
  declaredComponentsNames.forEach((compName: string) => {
    ...
    if (componentInfo) {
      ...
      modules.forEach((otherModuleInfo) => {
        if (otherModuleInfo.name !== moduleInfo.name) {
          ...
          const otherImportedModulesNames =
            otherModuleDecoratorArgs?.imports?.map((imp: any) =>
              imp.getText().replace('Module', ''),
            ) || [];
          ...
          if (otherImportedModulesNames.includes(moduleInfo.name.replace('Module', ''))) {
            ...
            const otherDeclaredComponentsNames =
              otherModuleDecoratorArgs?.declarations?.map((dec: any) =>
                dec.getText().replace('Component', ''),
              ) || [];
            if (!otherDeclaredComponentsNames.includes(compName)) {
              console.error(
                `خطا: کامپوننت '${componentInfo.name}' در ماژول '${moduleInfo.name}' تعریف شده است، اما در ماژول '${otherModuleInfo.name}' استفاده نشده است.`,
              );
            }
          }
        }
  ...
  importedModulesNames.forEach((importedModuleName: string) => {
    ...
    if (importedModuleInfo) {
      ...
      const importedModuleDeclaredComponentsNames =
        importedModuleDecoratorArgs?.declarations?.map((dec: any) =>
          dec.getText().replace('Component', ''),
        ) || [];
